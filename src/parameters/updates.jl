...
function updatesinglets!(
    ...
    for n in eachindex(MS.ds)
        for i in eachindex(MS.ds[n])
            ...
            st_ind = mapping.shift.st[n][i]

            MS.ds[n][i] = p[begin+st_ind-1]
        end
    end
    ...
    for n in eachindex(MS.βs)
        for i in eachindex(MS.βs[n])
            ...
            st_ind = mapping.phase.st[n][i]
            
            MS.βs[n][i] = p[begin+st_ind-1]
        end
    end
    ...
    for n in eachindex(MS.ξs)
        for i in eachindex(MS.ξs[n])
            ...
            st_ind = mapping.T2.st[n][i]

            MS.ξs[n][i] = p[begin+st_ind-1]
        end
    end
...
function updatespinsystems!(
    ...
    NMRSignalSimulator.updateparameters!(MSS.T2s, p, mapping.T2)
    NMRSignalSimulator.updateparameters!(MSS.shifts, p, mapping.shift)
    NMRSignalSimulator.updateparameters!(MSS.phases, p, mapping.phase)
